    ...
    <div>
        <v-card>
            <v-toolbar card prominent color="primary" dark="">
                <v-toolbar-title class="body-2">Buscar Servicios</v-toolbar-title>
            </v-toolbar>
            <v-card-text>
                <v-form v-model="valid" ref="form" lazy-validation>
                    <v-layout row wrap>
                        <v-flex lg10 sm10>
                            <v-text-field label="Servicio" name="servicio" v-model="search">
                            </v-text-field>
                        </v-flex>
                        <v-flex lg2>
...
<script>
import eliminarForm from '@/components/servicio/eliminarForm'
...
export default {
  components: {
    ...
    methods: {
        ...
        onSearch() {
          if(this.search != '') {
            axios.get('http://173.16.1.67:1337/servicio/' + this.search)
              .then(result => (this.basic.items = result.data))
              .catch(e => console.log(e));
          } else {
            axios.get('http://173.16.1.67:1337/servicios')
              .then(result => (this.basic.items = result.data))
              .catch(e => console.log(e));
          }
        }
