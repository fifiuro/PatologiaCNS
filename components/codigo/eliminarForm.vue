<template>
    <v-dialog v-model="basic.dialog" persistent max-width="500px">
        ...
        <v-card>
            <v-card-title>
                <span class="headline">Eliminar Código Beneficiario</span>
            </v-card-title>
            <v-divider></v-divider>
            <v-card-text>
                <v-container grid-list-md>
                    <h3>¿Esta seguro de eliminar al Código Beneficiario?</h3>
                </v-container>
            </v-card-text>
            <v-card-actions>
                <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    onDelete() {
      axios.delete('http://173.16.1.67:1337/codigos/' + this.id)
