  ...
  <div>
    <v-card>
      <v-toolbar card prominent color="primary" dark="">
        <v-toolbar-title class="body-2">Buscar Código de Beneficiario</v-toolbar-title>
        </v-toolbar>
        <v-card-text>
          <v-form v-model="valid" ref="form" lazy-validation>
            <v-layout row wrap>
              <v-flex lg10 sm10>
                <v-text-field label="Código de Beneficiario" name="codigo" v-model="search">
                </v-text-field>
              </v-flex>
              <v-flex lg2>
      ...
      <v-flex sm12>
        <h3>Resultado Código de Beneficiarios</h3>
      </v-flex>
      <v-flex lg12>
        <v-data-table
...
<script>
import eliminarForm from '@/components/codigo/eliminarForm'
...
export default {
  components: {
    ...
    methods: {
      onAdd() {
        this.$router.push('/codigo/nuevo');
      },
      onEdit(id) {
        this.$router.push('/codigo/' + id);
      },
      onSearch() {
        if (this.search != ""){
          axios.get('http://173.16.1.67:1337/codigo/' + this.search)
            .then(result => (this.basic.items = result.data))
            .catch(e => console.log(e));
        } else {
          axios.get('http://173.16.1.67:1337/codigos/')
            .then(result => (this.basic.items = result.data))
            .catch(e => console.log(e));
        }
      }
